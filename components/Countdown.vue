<template>
	<vue-countdown tag="div" class="bg-primary h-80 bg-opacity-80 text-center rounded-lg mb-6 flex justify-center items-center gap-3" :time="presaleTimeOffset" :interval="100" v-slot="{ days, hours, minutes, seconds, milliseconds }">
		<div class="flex flex-col text-center">
			<span class="text-6xl font-extrabold">{{ days }}</span>
			<span class="font-bold">Days</span>
		</div>
		<div class="text-6xl pt-3 font-extrabold pb-10">:</div>
		<div class="flex flex-col">
			<span class="text-6xl font-extrabold">{{ hours }}</span>
			<span class="font-bold">Hours</span>
		</div>
		<div class="text-6xl pt-3 font-extrabold pb-10">:</div>
		<div class="flex flex-col">
			<span class="text-6xl font-extrabold">{{ minutes }}</span>
			<span class="font-bold">Minutes</span>
		</div>
<!--		<div class="text-6xl pt-3 font-extrabold">:</div>-->
<!--		<div class="flex flex-col">-->
<!--			<span class="text-6xl font-extrabold">{{ seconds }}</span>-->
<!--			<span class="text-gray-400">Seconds</span>-->
<!--		</div>-->
	</vue-countdown>
</template>

<script>
export default {
	name: "Countdown",
	data() {
		return {
			now: new Date(),
			presaleDate: new Date("Wed, 21 Jan 2022 21:00:21 GMT"),
			endPresaleDate: new Date("Wed, 21 Jan 2022 21:00:21 GMT"),
			presaleTimeOffset: 0,
		}
	},
	mounted() {
		this.presaleTimeOffset = this.presaleDate - new Date();
	}
}
</script>

<style scoped>
.flex span {
//text-shadow: 0 0 0.3rem rgba(0, 0, 0, 0.1);
}
</style>
